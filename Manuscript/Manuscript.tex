...
\begin{figure}
	\centering
	\includegraphics[width=0.9\linewidth]{Figures/Pie_TotalEnergy}
	\caption{Total energy demand over one year of opeartions}
	\label{fig:Pie_TotalEnergy}
\end{figure}

The average HVAC daily demand, together with the average total electric power demand, is represented in Figure \ref{fig:PelHVAC_vs_time}. As it can be seen, the HVAC demand represents only a minor contribution to the total electric energy demand, as it is only present for a few months during summer, and at rather low power demand. This is not surprising, since it can be observed that (see Figure \ref{fig:Tair_cum}) the air is always below 27 degC, and 90\% of the time below 17 degC. 

\begin{figure}
	\centering
	\includegraphics[width=0.9\linewidth]{Figures/PelHVAC_vs_time}
	\caption{HVAC power demand versus total demand. Daily averages for one year of operations}
	\label{fig:PelHVAC_vs_time}
\end{figure}

\begin{figure}
	\centering
	\includegraphics[width=0.9\linewidth]{Figures/Tair_cum}
	\caption{Cumulative distribution of measured ambient air temperatures}
	\label{fig:Tair_cum}
\end{figure}
...
\begin{figure}
	...
	\label{fig:HeatGen_hist}
\end{figure}

Looking at the assumptions, this is mostly related to the assumption that the flow of heat recovery water is fixed to 297 m3/h. When the heat demand is medium or low, since the temperature BEFORE the consumers is fixed to 90 degrees, this limits the temperature drop on the consumers, making the maximum heat exchangeable in the HTHR heat exchangers to be very low. This can be seen in Figure \ref{fig:HTHR_T_hist}, where the histograms of the inlet temperature of the HT water and the HR water in the HTHR24 heat exchanger are plotted together. Since the maximum amount of heat that can be exchanged depends directly on this difference, the explanation is clear.

\begin{figure}
	\centering
	\includegraphics[width=0.99\linewidth]{Figures/HTHR_T_hist}
	\caption{Distribution of temperatures at the HTHR (24) inlet}
	\label{fig:HTHR_T_hist}
\end{figure}

The alternative solution would be to assume a constant temperature at the HTHR24 HR water inlet instead. This would, on the one hand, dramatically increase the contribution of the HT water to the heat demand of the ship. On the other hand, there is no information of what this temperature is, and little certainty that this is how the system is regulated.

Figure \ref{fig:Heat_comparison} shows the heat demand calculated based on different values assumed for the HTHR inlet temperature and of the reference outside temperature for the design demand of the HVAC preheater. The values of the average error (calculated both as absolute value and not) are given in Table \ref{tab:Heat_comparison}

\begin{figure}
	\centering
	\includegraphics[width=0.99\linewidth]{Figures/Heat_comparison}
	\caption{Comparison of the calculated steam demand for different values of the fixed HTHR inlet temperature and of the reference temperature for the design HVAC preheater load. The shaded green area represents the time frame when no heating occurs}
...
\begin{table}
	...
	\begin{tabular}{cccc}
		\hline 
		\rule[-1ex]{0pt}{2.5ex} $T_{HTHR,in} $ & $T_{OUT,des} $ & $\epsilon$ & $\epsilon_{abs}$ \\ 
		\hline 
		\rule[-1ex]{0pt}{2.5ex} 70 & -20 & 100\% & 80\% \\ 
		\hline 
		\rule[-1ex]{0pt}{2.5ex} 65 & -20 & 76\% & 67\% \\ 
		\hline 
		\rule[-1ex]{0pt}{2.5ex} 60 & -20 & 57\% & 57\% \\ 
		\hline 
		\rule[-1ex]{0pt}{2.5ex} 55 & -20 & 42\% & 49\% \\ 
		\hline 
		\rule[-1ex]{0pt}{2.5ex} 50 & -20 & 28\% & 42\% \\ 
		\hline 
		\rule[-1ex]{0pt}{2.5ex} 70 & -30 & 70\% & 69\% \\ 
		\hline 
		\rule[-1ex]{0pt}{2.5ex} 50 & -30 & 8\% & 35\% \\ 
		\hline 
		\rule[-1ex]{0pt}{2.5ex} $\dot{m}=K$ = k & -30 & 207\% & 147\% \\ 
		\hline 
	\end{tabular} 
	\label{tab:Heat_comparison}
\end{table}
