...
for j = 1 : n_data    
    ...
    if ME_on.(char(ME_names(engine_index)))(j) > 0  
        ...
        temp_bsfc_ISO(local_index) = polyval(ME_POLY_LOAD_2_ISO_BSFC,temp_load(local_index)) ;
        temp_bsfc(local_index) = bsfcISOCorrection(temp_bsfc_ISO(local_index) , ME_T_LTcooling.(char(ME_names(engine_index)))(j) , ME_T_charge_air.(char(ME_names(engine_index)))(j) , LHV , 0.8) ;
        temp_mfr_fuel(j,local_index) = temp_load(local_index) * temp_bsfc(local_index) / temp_bsfc_ISO(local_index) * ME_MFR_FUEL_DES_ISO ;
    end
    %% ME 3
    engine_index = 3 ;
    local_index = 2 ; 
    ...
    clear temp_load temp bsfc_ISO temp_bsfc
end
